...
describe.only('kad-dht', () => {
  ...
  describe('kad-dht with connections per peer', () => {
    ...
    describe.skip('a JS node in the land of Go - libp2p-record', () => {
      let nodes = []
      ...
      before(function (done) {
        ...
        parallel(nodes.map((node) => (cb) => node.api.id(cb)), (err, ids) => {
          expect(err).to.not.exist()
          parallel([
            (cb) => nodes[3].api.swarm.connect(ids[1].addresses[0], cb), // 1
            (cb) => nodes[0].api.swarm.connect(ids[2].addresses[0], cb), // 2
            (cb) => nodes[1].api.swarm.connect(ids[3].addresses[0], cb) // 3 to simulate libp2p-record problem
          ], done)
        })
      })
